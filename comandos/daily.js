...
module.exports = {
 ...
 async execute(client, message, args, db, economy, MESSAGES_JSON, LANG) {
    ...
    if (cooldown.daily !== null && timeout - (Date.now() - cooldown.daily) > 0) {
     
        let time = timeout - (Date.now() - cooldown.daily)
      var ms = time % 1000;
       s = (time - ms) / 1000;
        var secs = s % 60;
        s = (s - secs) / 60;
       ...
       var hrs = s % 24
        ...
        message.channel.send(new Discord.MessageEmbed().setDescription(MESSAGES_JSON[LANG].MESSAGE_DAILY_EMBED_ALREADY_DESCRIPTION+`**${hrs}h ${mins}m ${secs}s**`).setColor("RED"))
    } else {
      balance.money += amount
  
      console.log(cooldown)
        await balance.save()
         cooldown.daily = Date.now()
