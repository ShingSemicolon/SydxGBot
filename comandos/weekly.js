...
module.exports = {
 ...
 async execute(client, message, args, db, economy, MESSAGES_JSON, LANG) {
    ...
    let amount = Math.floor(Math.random() * 40) + 90;
    ...
    if (cooldown.weekly !== null && timeout - (Date.now() - cooldown.weekly) > 0) {
   
        let time = timeout - (Date.now() - cooldown.weekly)
          var ms = time % 1000;
       s = (time - ms) / 1000;
        var secs = s % 60;
        s = (s - secs) / 60;
       ...
       var hrs = s % 24
        var days = (s - hrs) / 24
        message.channel.send(new Discord.MessageEmbed().setDescription(MESSAGES_JSON[LANG].MESSAGE_WEEKLY_EMBED_ALREADY_DESCRIPTION+`**${days}d ${hrs}h ${mins}m ${secs}s**`).setColor("RED"))
    } else {
        balance.money += amount
       await balance.save()
         cooldown.weekly = Date.now()
         await cooldown.save()
